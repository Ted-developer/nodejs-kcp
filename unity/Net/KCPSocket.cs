...
using Timer = WBFramework.Timer;
using Debugger = WBFramework.Debugger;
using XLua;
...
namespace WBFramework {
    ...
    public class CommSocket : SocketTask {
        ...
        public CommSocket (string url) {
            ...
            this._url = $"ws://{this._url}/"; // todo(chentao) wss
        }
    ...
    public class KcpSocket : SocketTask {
        ...
        public KcpSocket (string url, bool enableUdp, int timeoutSec = 60) {
            ...
            this._udpClient = new KcpClient (41235);

            _recvTimeoutSec = timeoutSec;
            _udpRcvBuf = new byte[(Kcp.IkcpMtuDef + Kcp.IkcpOverhead) * 3];
        ...
        public override void Connect () {
            try {
                ...
                if (_udpClient.Connected ()) {
                    _kcp = new Kcp (123, this);
                    _kcp.SetOutput (OutputKcp);

                    // fast mode
                    _kcp.NoDelay (1, 100, 2, 1);
                    _kcp.WndSize (1024, 1024);

                    Timer.SetTimer (() => StartKcpUpdate (), 20); // todo(chentao) 20支持可配置
